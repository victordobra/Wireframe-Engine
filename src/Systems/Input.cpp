...
namespace mge {
    ...
    static MousePos InternalGetMousePos() {
        ...
        if(!result) {
            ...
            mge::console::OutFatalError((string)"Failed to retrieve the cursor's position! Error code: " + ToString(errorCode), 1);
        }
    ...
    void UpdateInputValues() {
        ...
        if(mouseState == MOUSE_STATE_LOCKED) {
            ...
            if(!result) {
                sint32_t errorCode = GetLastError();
                mge::console::OutFatalError((string)"Failed to set the cursor's position! Error code: " + ToString(errorCode), 1);
            }
#endif
        }
    }
