...
namespace wfe {
    ...
    struct NodeType {
        struct Property {
            ...
            static Property GetPropertyInfo() {
                ...
                if(prop.hashCode == WFE_TYPE_ID(sint16_t).Hash64() || prop.hashCode == WFE_TYPE_ID(sint32_t).Hash64() || prop.hashCode == WFE_TYPE_ID(sint64_t).Hash64())
    ...
    public:
        ...
        template<class T>
        static T* CreateNode() {
            NodeType* nodeType = GetNodeType(WFE_TYPE_ID(T).Hash64());
...
// Macros
#define WFE_NODE(type)  \
namespace { \
    typedef type T; \
    /* Function that creates a node of the specified type and returns it. */ \
    wfe::Node* Create ## type ## Node() { \
        wfe::Node* node = dynamic_cast<wfe::Node*>(new type()); \
        if(!node) \
            console::OutFatalError("Failed to convert object to node type!", 1); \
        node->nodeType = wfe::Node::GetNodeType(WFE_TYPE_ID(type).Hash64()); \
        return node; \
    } \
 \
    struct type ## NodeTypeInitializer { \
        type ## NodeTypeInitializer() { \
            wfe::NodeType nodeType; \
 \
            nodeType.name = #type; \
            nodeType.hashCode = WFE_TYPE_ID(type).Hash64(); \
            nodeType.create = Create ## type ## Node; \
 \
            type* ptr = nullptr;

#define WFE_ADD_NODE_PROPERTY(propName, accessInfo) \
            wfe::NodeType::Property propName ## Property = wfe::NodeType::Property::GetPropertyInfo<decltype(ptr->propName)>(); \
            propName ## Property.offset = (uint64_t)&(ptr->propName); \
            propName ## Property.access = wfe::NodeType::Property::PROPERTY_ACCESS_ ## accessInfo; \
            nodeType.properties.push_back(propName ## Property);

#define WFE_END_NODE(type) \
            wfe::Node::nodeTypes[wfe::Node::nodeTypeCount++] = nodeType; \
        } \
    }; \
 \
